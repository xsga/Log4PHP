...
final class LoggerPatternConverterDate extends LoggerPatternConverter
{
    ...
    public function activateOptions(): void
    {
        if ($this->option !== null && $this->option !== '') {
            $this->format = match (isset($this->specials[$this->option])) {
                true  => $this->specials[$this->option],
                false => $this->option,
            };
        }
    ...
    private function date(string $format, float $utimestamp): string
    {
        ...
        $ms        = floor(($utimestamp - $timestamp) * 1000.00);
